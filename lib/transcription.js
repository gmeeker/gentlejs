...
class Word {
  ...
  static NOT_FOUND_IN_AUDIO() { return 'not-found-in-audio'; }

  static NOT_FOUND_IN_TRANSCRIPT() { return 'not-found-in-transcript'; }
  ...
  toString() {
    ...
    return 'Word(' + keys(d).sort().map(key => `${key}=${d[key]}`).join(' ') + ')';
  }
...
class Transcription {
  ...
  toJSON(options = {}) {
    ...
    return stringify(container, { space: 4, ...options });
  }
