...
const disfluencies = { uh: 'uh', um: 'um' };
...
const transcript = fs.readFileSync(args.txtfile);

const resources = gentle.Resources();
logging.info('converting audio to 8K sampled wav');

const wavfile = gentle.resampled(args.audiofile);
const aligner = gentle.ForcedAligned({
  resources,
  transcript,
...
const result = aligner.transcribe(wavfile, { progress: onProgress, logging });

let fh = process.stdout;
if (args.output) {
  fh = fs.createWriteStream(args.output);
}
fh.write(JSON.stringify(result, null, 2));
if (args.output) {
