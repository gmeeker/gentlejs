...
class MultiThreadedTranscriber {
  ...
  async transcribe(wavfile, progressCb) {
    ...
    return fs.stat(wavfile).then(stat => {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.on('readable', async () => {
          let data;
          while (data !== null) {
            data = reader.read(this.chunkLen * byteRate);
            if (data) {
            ...
            while (data === null || this.buffer.length >= this.chunkLen * byteRate) {
              if (data === null && this.buffer.length < 4000 * 2) {
                ...
                const k = await this.kaldiQueue.get();
                k.pushChunk(buf);
