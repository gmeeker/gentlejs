...
class Realigner extends WavBuffer {
  ...
  async ready(data) {
    ...
    const chunkGenHclgFilename = await makeBigramLanguageModel(chunkKs, resources.protoLangDir);
    return this.kaldiQueue.get().then(() => {
      ...
      k.start().then(() => {
        ...
        this.kaldiQueue.put(null, k.getFinal().then(final => {
          ...
          if (this.progressCb) {
            this.progressCb({ percent: this.realignments.length / this.toRealign.length });
          }
        }));
      });
    });
