...
class ForcedAligner {
  constructor(resources, transcript, nthreads = 4, options = {}) {
    this.options = options;
    this.nthreads = nthreads;
    this.transcript = transcript;
    this.resources = resources;
    this.ms = MetaSentence(transcript, resources.vocab);
    const ks = this.ms.getKaldiSequence();
    const genHclgFilename = makeBigramLanguageModel(ks, resources.protoLangdir, options);
    this.queue = kaldiQueue.build(resources, genHclgFilename, nthreads);
    this.mtt = MultiThreadedTranscriber(this.queue, { nthreads });
  }

  transcribe(wavfile, progressCb, logging) {
    const result = this.mtt.transcribe(wavfile, progressCb);
    let { words } = result;
    const { duration } = result;

    // Clear queue (would this be gc'ed?)
    for (let i = 0; i < this.nthreads; i++) {
      const k = this.queue.get();
      k.stop();
    }
    ...
    if (progressCb) {
      progressCb({ status: 'ALIGNING' });
    }

    words = realign(wavfile, words, this.ms, this.resources, this.nthreads, progressCb);
    ...
    return new Transcription(words, this.transcript);
  }
}
